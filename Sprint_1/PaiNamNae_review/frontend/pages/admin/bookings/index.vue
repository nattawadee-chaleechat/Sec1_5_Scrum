<template>
  <div>
          ...
          >
            ...
            <div class="lg:col-span-5">
              <label class="block mb-1 text-xs font-medium text-gray-600"
                >จุดรับ (ใช้ key: name)</label
              >
              <input
                v-model.trim="filters.pickupName"
                type="text"
                placeholder="เช่น มอแดง เพลส"
            ...
            <div class="lg:col-span-5">
              <label class="block mb-1 text-xs font-medium text-gray-600"
                >จุดส่ง (ใช้ key: name)</label
              >
              <input
                v-model.trim="filters.dropoffName"
                type="text"
                placeholder="เช่น โลตัส ขอนแก่น 2"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500"
              />
            </div>
            ...
            <div class="lg:col-span-5">
              <label class="block mb-1 text-xs font-medium text-gray-600"
                >เวลาออกเดินทางตั้งแต่ (YYYY-MM-DD)</label
              >
              <input
            ...
            <div class="lg:col-span-4">
              ...
              >
                ...
                <option value="route.departureTime:desc">
                  departureTime desc
                </option>
                <option value="route.departureTime:asc">
                  departureTime asc
                </option>
              </select>
            </div>
            ...
            <!-- Actions (2/24) -->
            <div
              class="flex items-end justify-end gap-2 mt-1 lg:col-span-4 lg:mt-0"
            >
              <button
                @click="clearFilters"
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                >
                  ...
                  <td class="px-4 py-3">
                    ...
                    <div class="text-xs text-gray-500">
                      ไดรเวอร์: {{ b.route?.driver?.firstName }}
                      {{ b.route?.driver?.lastName }}
                      <span class="mx-2 text-gray-300">|</span>
                      รถ: {{ b.route?.vehicle?.vehicleModel || "-" }}
                      <span class="text-gray-400"
                        >({{ b.route?.vehicle?.vehicleType || "-" }})</span
                      >
                    </div>
                  </td>
                  ...
                  <td class="px-4 py-3 text-gray-700">
                    <div class="text-sm">
                      {{ formatDate(b.route?.departureTime) }}
                    </div>
                  </td>
                  ...
                  <td class="px-4 py-3 text-gray-700">
                    ...
                    <div class="text-xs text-gray-500">
                      อัปเดต {{ formatDate(b.route?.updatedAt) }}
                    </div>
                  </td>
    ...
    <ConfirmModal
      :show="showDelete"
      :title="`ลบการจอง${deletingBooking?.id ? ' : ' + deletingBooking.id : ''}`"
