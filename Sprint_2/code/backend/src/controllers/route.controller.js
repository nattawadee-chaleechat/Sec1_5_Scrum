...
const createRoute = asyncHandler(async (req, res) => {
  ...
  if (primary) {
    ...
    payload.routeSummary =
      primary.summary ||
      `${legs[0]?.start_address} → ${legs.at(-1)?.end_address}`;
    payload.distance = legs.length
      ? legs
          .map((l) => l.distance?.text)
          .filter(Boolean)
          .join(" + ")
      : null; // หรือ format เอง
    payload.duration = legs.length
      ? legs
...
const updateRoute = asyncHandler(async (req, res) => {
  ...
  const hasBookings =
    Array.isArray(existing.bookings) && existing.bookings.length > 0;
  if (hasBookings) {
    const hasConfirmed = existing.bookings.some(
      (b) => b.status === "CONFIRMED",
    );
    if (hasConfirmed) {
      throw new ApiError(
        400,
        "ไม่สามารถแก้ไขเส้นทางได้ เนื่องจากมีคำจองที่ยืนยันแล้ว (CONFIRMED)",
      );
    }
    ...
    if (!allAllowed) {
      throw new ApiError(
        400,
        "ไม่สามารถแก้ไขเส้นทางได้ เนื่องจากมีคำจองที่อยู่ในสถานะที่ไม่อนุญาต",
      );
    }
  }
  ...
  const startChanged =
    routeFields.startLocation !== undefined &&
    JSON.stringify(routeFields.startLocation) !==
      JSON.stringify(existing.startLocation);
  const endChanged =
    routeFields.endLocation !== undefined &&
    JSON.stringify(routeFields.endLocation) !==
      JSON.stringify(existing.endLocation);
  const timeChanged = routeFields.departureTime !== undefined;
  const wpsChanged =
    routeFields.waypoints !== undefined &&
    JSON.stringify(routeFields.waypoints) !==
      JSON.stringify(
        Array.isArray(existing.waypoints)
          ? existing.waypoints
          : existing.waypoints?.requested || [],
      );
  const optimizeChanged =
    routeFields.optimizeWaypoints !== undefined &&
  ...
  if (
    ...
    const depTime = (
      payload.departureTime ?? existing.departureTime
    ).toISOString();
    const currentWps =
      routeFields.waypoints !== undefined
...
const adminCreateRoute = asyncHandler(async (req, res) => {
  ...
  if (!approved) {
    throw new ApiError(
      400,
      "ไม่สามารถสร้างเส้นทางให้ไดรเวอร์ที่ยังไม่ได้ยืนยันตัวตน (ต้องมีรายการยืนยันและสถานะไม่เป็น REJECTED)",
    );
  }
