<template>
  <header
    class="fixed top-0 left-0 right-0 z-50 h-16 bg-white border border-gray-300 shadow-sm"
  >
    <div class="flex items-center justify-between h-full px-4">
      <div class="flex items-center gap-4">
        ...
        <button
          ...
          class="text-gray-600 lg:hidden hover:text-blue-600"
        >
        ...
        <div class="flex items-center gap-2">
          <div
            class="flex items-center justify-center w-8 h-8 bg-blue-600 rounded"
          >
                  ...
                  <div class="px-4 py-3 hover:bg-gray-50">
                    <div class="flex items-start gap-3">
                      ...
                      <span
                        ...
                        :class="
                          n.adminReviewedAt ? 'bg-gray-300' : 'bg-emerald-500'
                        "
                      />
                      <!-- เนื้อหา -->
...
const unreadCount = computed(
  () => notifications.value.filter((n) => !n.adminReviewedAt).length,
);
...
async function fetchAdminNotifications() {
  try {
    if (!user?.value || String(user.value.role).toUpperCase() !== "ADMIN")
      return;
    loading.value = true;
    ...
    const list = raw.filter(
      (it) => (it?.metadata?.initiatedBy || "").toLowerCase() === "user",
    );
...
async function markAsRead(n) {
  try {
    ...
    if (i > -1)
      notifications.value[i].adminReviewedAt = new Date().toISOString();
  } finally {
...
onMounted(() => {
  ...
  if (String(user?.value?.role || "").toUpperCase() === "ADMIN")
