          ...
          <div class="bg-white border border-gray-300 rounded-lg shadow-md">
            <div class="p-6 border-b border-gray-300">
              <h3 class="text-lg font-semibold text-gray-900">
                ผลการค้นหา ({{ routes.length }} รายการ)
              </h3>
            </div>
            ...
            <div v-else class="divide-y divide-gray-200">
              ...
              >
                <h1 class="mb-6 font-semibold text-center text-gray-900">
                  {{ route.originName }}
                  <span class="mx-1 font-semibold text-gray-900">→</span>
                  {{ route.destinationName }}
                </h1>
                <div class="flex items-start space-x-4">
                  <img
                    :src="route.driver.image"
                  ...
                  <div class="flex-1">
                    <div class="flex items-start justify-between">
                      <div>
                        ...
                        <div
                          ...
                          @click.stop="openReviewModal(route)"
                        >
                    ...
                    <div class="mt-3">
                      <div
                        class="flex flex-wrap items-center text-sm text-gray-600"
                      >
                ...
                <div
                  v-if="selectedRoute && selectedRoute.id === route.id"
            ...
            <div class="flex flex-col items-center">
              <img
                :src="
                  ...
                  driverInfo?.driver?.profilePicture
                "
        ...
        <div class="modal-content">
          ...
          <template v-else>
            ...
            >
              <h3 class="text-lg font-semibold text-gray-900">
                เลือก{{
                  bookingPickingTarget === "pickup" ? "จุดขึ้นรถ" : "จุดลงรถ"
                }}
              </h3>
              <button
                @click="stopBookingPicker"
                class="text-gray-400 hover:text-gray-600"
              >
                <svg
                  class="w-6 h-6"
          ...
          <div class="p-6">
            <div class="mb-6">
              ...
              >
                <img
                  :src="bookingRoute.driver.image"
                  :alt="bookingRoute.driver.name"
                  class="object-cover w-12 h-12 rounded-full"
                />
                <div>
                  ...
                  <div class="flex items-center">
                    <div class="font-medium text-gray-900">
                      {{ bookingRoute.driver.name }}
                    </div>

                    <div
                      v-if="bookingRoute.driver.isVerified"
                      class="relative group ml-1.5 flex items-center"
                    >
                      <svg
                        class="w-5 h-5 text-blue-600"
            ...
            <div class="mb-6">
              ...
              <div
                class="flex items-center p-3 space-x-4 rounded-lg bg-gray-50"
              >
                <div class="flex-1">
                  <div class="font-medium text-gray-900">
                    {{ bookingRoute.originName }}
                  </div>
            ...
            <div class="mb-6 space-y-4">
              <div>
                <label class="block mb-2 text-sm font-medium text-gray-700"
                  >จำนวนที่นั่งที่ต้องการจอง</label
                >
                <select
                ...
                >
                  <option
                    v-for="seat in bookingRoute.availableSeats"
                    :key="seat"
                    :value="seat"
                  >
                    {{ seat }} ที่นั่ง
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <button
                    type="button"
                    @click="startBookingPicker('pickup')"
                    class="absolute inset-y-0 my-auto text-gray-500 right-2 hover:text-blue-600"
                    title="เลือกจากแผนที่"
                  >
                    <svg
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <button
                    type="button"
                    @click="startBookingPicker('dropoff')"
                    class="absolute inset-y-0 my-auto text-gray-500 right-2 hover:text-blue-600"
                    title="เลือกจากแผนที่"
                  >
                    <svg
            ...
            <div class="p-4 mb-6 rounded-lg bg-blue-50">
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="font-medium text-gray-900"
                  >{{ bookingRoute.price }} บาท</span
                >
              </div>
              <div class="flex items-center justify-between mb-2">
                <span class="text-gray-700">จำนวนที่นั่ง</span>
                <span class="font-medium text-gray-900"
                  >{{ bookingSeats }} ที่นั่ง</span
                >
              </div>
      ...
      >
        <div class="modal-content">
          ...
          >
            <h3 class="text-lg font-semibold text-gray-900">
              เลือก{{
                pickingField === "origin" ? "จุดเริ่มต้น" : "จุดปลายทาง"
              }}
            </h3>
            <button
          ...
          <div class="p-4 space-y-3">
            <div
              ref="placePickerMapEl"
              class="w-full border border-gray-200 rounded-md h-80"
            ></div>
            <div class="text-sm text-gray-700">
              <div class="font-medium">ตำแหน่งที่เลือก:</div>
              <div class="truncate">
                {{ pickedPlace.name || "— ยังไม่เลือก —" }}
              </div>
              <div
                v-if="pickedPlace.lat && pickedPlace.lng"
                class="text-gray-500"
              >
                lat: {{ pickedPlace.lat.toFixed(6) }}, lng:
                {{ pickedPlace.lng.toFixed(6) }}
              </div>
            </div>
            <div class="flex gap-3 pt-2">
              ...
              <button
                :disabled="!pickedPlace.name"
                @click="applyPickedPlace"
...
async function handleSearch() {
  ...
  try {
    ...
    if (
      ...
      searchForm.value._destinationMeta?.lat
    ) {
    ...
    const raw = (apiRes?.data || apiRes || []).filter(
    ...
    routes.value = raw.map((route) => {
      ...
      return {
        ...
        originName:
          ...
          `(${route.startLocation.lat.toFixed(2)}, ${route.startLocation.lng.toFixed(2)})`,
        destinationName:
          route.endLocation?.name ||
          `(${route.endLocation.lat.toFixed(2)}, ${route.endLocation.lng.toFixed(2)})`,
        originAddress: route.startLocation?.address
          ? cleanAddr(route.startLocation.address)
          : null,
        destinationAddress: route.endLocation?.address
          ? cleanAddr(route.endLocation.address)
          : null,
        driver: {
          id: route.driver?.id,
          name:
            ...
            "ไม่ระบุชื่อ",
          image:
            route.driver?.profilePicture ||
            `https://ui-avatars.com/api/?name=${encodeURIComponent(route.driver?.firstName || "U")}&background=random&size=64`,
        ...
        carDetails: route.vehicle
          ? [
              `${route.vehicle.vehicleModel} (${route.vehicle.vehicleType})`,
              ...(route.vehicle.amenities || []),
            ]
          : ["ไม่มีข้อมูลรถ"],
...
async function hydrateDriverRatings(routeList) {
  const tasks = routeList
    ...
    .filter(
      (r) =>
        !Number.isFinite(r.driver.rating) ||
        r.driver.rating === 0 ||
        r.driver.reviews == null,
    )
    .map(async (route) => {
...
async function updateMapForRoute(route) {
  ...
  if (Array.isArray(route.stopsCoords) && route.stopsCoords.length) {
    stopMarkers = route.stopsCoords.map(
      (s, idx) =>
...
async function formatPrettyAddress(geocodeResult) {
  ...
  if (isPoi && geocodeResult.place_id) {
    ...
    if (poiName) {
      ...
      const province =
        byType("administrative_area_level_1") ||
        byTypeShort("administrative_area_level_1");
      if (sublocality && province)
        return `${poiName}, ${sublocality}, ${province}`;
  ...
  if (street && sublocality && province)
    return `${street}, ${sublocality}, ${province}`;
...
async function confirmBooking() {
  ...
  if (!pickupData.value.lat || !dropoffData.value.lat) {
    toast.warning(
      "ข้อมูลไม่ครบถ้วน",
      "กรุณาเลือกจุดขึ้นรถและจุดลงรถจากรายการหรือปักหมุดบนแผนที่",
    );
    return;
  }
...
function initAutocomplete() {
  ...
  originAutocomplete.addListener("place_changed", () => {
    ...
    searchForm.value.origin =
      p.name || p.formatted_address || searchForm.value.origin;
  ...
  destinationAutocomplete.addListener("place_changed", () => {
    ...
    if (!p) return;
    searchForm.value.destination =
      p.name || p.formatted_address || searchForm.value.destination;
...
async function resolvePicked(latlng) {
  ...
  if (!name || isPlusCode(name)) {
    ...
    if (poi?.name) {
      ...
      name = geocodeRes.formatted_address.replace(
        /,?\s*(Thailand|ไทย)\s*$/i,
        "",
      );
    }
...
function startBookingPicker(target) {
  ...
  nextTick(() => {
    ...
    const center =
      base.lat && base.lng ? { lat: base.lat, lng: base.lng } : fallbackCenter;
...
function setBookingPickerMarker(latlng) {
  ...
  bookingPickerMarker = new google.maps.Marker({
    position: latlng,
    map: bookingPickerMap,
    draggable: true,
  });
  bookingPickerMarker.addListener("dragend", (e) =>
    resolveBookingPicked(e.latLng),
  );
}
async function resolveBookingPicked(latlng) {
  ...
  let name = "",
    placeId = geocodeRes?.place_id || null,
    address = geocodeRes?.formatted_address || null;
  if (geocodeRes) {
    const parts = await extractNameParts(geocodeRes);
    name = parts.name || "";
  }
    ...
    if (poi?.place_id) {
      ...
      const details = await new Promise((resolve) => {
        placesService.getDetails(
          { placeId: poi.place_id, fields: ["name", "formatted_address"] },
          (pl, status) =>
            resolve(
              status === google.maps.places.PlacesServiceStatus.OK ? pl : null,
            ),
        );
      });
  ...
  try {
    ...
    if (response) {
      driverInfo.value = {
        ...
        profilePicture:
          response.profilePicture || driverInfo.value?.driver?.image,
        ...
        driver: {
          ...
          reviews:
            response.driver?.reviews ?? driverInfo.value?.driver?.reviews,
        },
      };
