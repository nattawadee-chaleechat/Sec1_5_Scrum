...
const createBookingSchema = z.object({
  ...
  //[Description] เพิ่ม selectedCharges chargeQuantities เพื่อไม่ให้โดน validation กันออกตอนส่งข้อมูล
  selectedCharges: z.array(z.string()).optional(),
  chargeQuantities: z.record(z.string(), z.number()).optional(),
});
...
const listBookingsQuerySchema = z.object({
  ...
  createdFrom: z
    .string()
    .refine((v) => !isNaN(Date.parse(v)), { message: "Invalid createdFrom" })
    .optional(),
  createdTo: z
    .string()
    .refine((v) => !isNaN(Date.parse(v)), { message: "Invalid createdTo" })
    .optional(),
  routeDepartureFrom: z
    .string()
    .refine((v) => !isNaN(Date.parse(v)), {
      message: "Invalid routeDepartureFrom",
    })
    .optional(),
  routeDepartureTo: z
    .string()
    .refine((v) => !isNaN(Date.parse(v)), {
      message: "Invalid routeDepartureTo",
    })
    .optional(),
...
const updateBookingByAdminSchema = z
  .object({
