...
// [26/2/2569]
// - ปรับ getDriverReview ให้รองรับ media (image, video, audio, link)
// - ลบ logic เก่า picture → ใช้ media array แทน
// - แก้โครงสร้าง select และ orderBy ให้ถูกต้อง
// - ปรับ response format ให้แยกประเภทสื่อชัดเจน
  ...
  return reviews.map(r => {
    ...
    return {
      ...
      images: Array.isArray(r.media)
        ? r.media.filter(m => m.type === 'image')
        : [],

      videos: Array.isArray(r.media)
        ? r.media.filter(m => m.type === 'video')
        : [],

      audio: Array.isArray(r.media)
        ? r.media.filter(m => m.type === 'audio')
        : [],

      links: Array.isArray(r.media)
        ? r.media.filter(m => m.type === 'link')
        : [],
