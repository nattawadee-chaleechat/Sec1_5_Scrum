        ...
        >
              ...
              <div>
                ...
                <div class="relative">
                  <input
                    ...
                    name="endPoint"
                    type="text"
                    placeholder="เช่น เชียงใหม่, ถนนนิมมานเหมินท์"
                    required
                    class="w-full px-4 py-3 pr-10 border border-gray-300 rounded-md focus:ring-blue-500"
                  />
                  <button
                    type="button"
                    @click="openPlacePicker('end')"
                    class="absolute inset-y-0 my-auto text-gray-500 right-2 hover:text-blue-600"
                    title="เลือกจากแผนที่"
                  >
                    <svg
            ...
            <div class="mt-6">
              ...
              <div class="space-y-4">
                <div v-for="(wp, idx) in waypoints" :key="idx" class="relative">
                  ...
                  />
                  <div
                    class="absolute inset-y-0 flex items-center gap-2 my-auto right-2"
                  >
                    <button
                      type="button"
                      class="text-gray-500 hover:text-blue-600"
                      @click="openPlacePicker(`wp-${idx}`)"
                      title="เลือกจากแผนที่"
                    >
                      <svg
                        class="w-5 h-5"
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  type="number"
                  min="1"
                  placeholder="กรอกจำนวนที่นั่ง (เช่น 4)"
                  required
                  class="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring-blue-500"
                />
          ...
          <div>
            ...
            <div
              ...
              class="grid items-center grid-cols-1 gap-6 md:grid-cols-2"
            >
              <div>
                <label
                  for="vehicle"
            ...
            <div
              v-else
              class="p-8 bg-white border border-gray-300 shadow-xl rounded-xl"
            >
              <div
                class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-[#F2F2F2] p-4 md:px-6 md:py-6 rounded-[8px]"
              >
                <p
                  class="text-gray-800 text-base md:text-[18px] text-center sm:text-left"
                >
          ...
          <div>
            ...
            <textarea
              ...
              class="w-full px-4 py-3 border border-gray-300 rounded-md resize-none focus:ring-blue-500"
            ></textarea>
          </div>
  ...
  try {
    ...
    if (userVehicles.length > 0) {
      const defaultVehicle =
        userVehicles.find((v) => v.isDefault) || userVehicles[0];
...
const handleSubmit = async () => {
  ...
  const waypointsPayload = waypointMetas.value
    .map((m, i) => {
      const name =
        (m && m.name) ||
        (waypoints.value[i] && waypoints.value[i].text) ||
        null;
      const address =
        (m && m.address) ||
        (waypoints.value[i] && waypoints.value[i].text) ||
  ...
  try {
    ...
    toast.success(
      "สำเร็จ",
      body?.message || "สร้างเส้นทางการเดินทางเรียบร้อยแล้ว!",
    );
    setTimeout(() => {
      navigateTo("/findTrip");
    }, 1500);
...
function initWaypointAutocomplete(idx) {
  ...
  ac.addListener("place_changed", () => {
    ...
    const name =
      p.name ||
      stripLeadingPlusCode(stripCountry(p.formatted_address || "")) ||
      waypoints.value[idx].text;
    const address = stripCountry(p.formatted_address || name || "");

    waypoints.value[idx].text = name;
    waypointMetas.value[idx] = {
      lat,
      lng,
      name,
      address,
  ...
  if (geocodeRes?.place_id) {
    await new Promise((done) => {
      placesService.getDetails(
        { placeId: geocodeRes.place_id, fields: ["name"] },
        (pl, st) => {
