<template>
  <div
    class="bg-gray-50 min-h-screen flex items-center justify-center p-4 font-['Kanit']"
  >
    <main class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg sm:p-8">
      <h1 class="mb-8 text-3xl font-bold text-center text-blue-600">
        สมัครสมาชิก
      </h1>
      ...
      <div class="relative mb-10">
        ...
        <div
          class="absolute left-0 top-5 h-0.5 bg-blue-600 transition-all duration-500"
        ...
        <div class="relative flex items-center justify-between">
          <div
            ...
            :key="step"
            class="z-10 flex flex-col items-center w-1/3"
          >
        ...
        <div v-if="currentStep === 1">
          ...
          <div class="mb-4">
            ...
            <p v-else class="mt-1 text-xs text-gray-500">
              ความยาว 4–20 ตัวอักษร ภาษาอังกฤษ ตัวเลข หรือขีดล่าง (_) เท่านั้น
            </p>
          </div>
          <div class="mb-4">
            <label
          ...
          <div class="mb-6">
            ...
            <input
              ...
              class="w-full px-4 py-2 transition border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.confirmPassword,
              }"
            />
        ...
        <div v-if="currentStep === 2">
          ...
          <div class="mb-4">
            ...
            <input
              ...
              class="w-full px-4 py-2 transition border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.firstName,
              }"
            />
          ...
          <div class="mb-4">
            ...
            <input
              ...
              class="w-full px-4 py-2 transition border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.phoneNumber,
              }"
            />
          ...
          <div class="flex gap-4">
            <button
              type="button"
              @click="prevStep"
              class="w-full py-3 font-medium text-white transition bg-gray-600 rounded-md hover:bg-gray-700"
            >
              ย้อนกลับ
            </button>
            <button
        ...
        <div v-if="currentStep === 3">
          ...
          <div class="mb-4">
            <label class="block mb-1 text-sm font-medium text-gray-700"
              >บัตรประชาชน <span class="text-red-500">*</span></label
            >
            <div
              v-if="!idCardPreview"
              @click="triggerFileUpload('idCardFile')"
              class="p-6 text-center transition border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.idCardFile,
              }"
            >
            ...
            <div v-else class="relative">
              ...
              <button
                type="button"
                @click="removeImage('idCard')"
                class="absolute flex items-center justify-center w-6 h-6 text-white bg-black bg-opacity-50 rounded-full top-2 right-2"
              >
                &times;
              </button>
            </div>
            <input
              ...
              class="hidden"
            />
            <p v-if="errors.idCardFile" class="mt-1 text-xs text-red-600">
              {{ errors.idCardFile }}
            </p>
          </div>
          <div class="mb-4">
            <label
              for="idNumber"
              class="block mb-1 text-sm font-medium text-gray-700"
              >หมายเลขบัตรประชาชน <span class="text-red-500">*</span></label
            >
            <input
              type="text"
              id="idNumber"
              v-model="formData.idNumber"
              placeholder="กรอกหมายเลข 13 หลัก"
              class="w-full px-4 py-2 transition border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{ 'border-red-500 ring-1 ring-red-500': errors.idNumber }"
            />
            <p v-if="errors.idNumber" class="mt-1 text-xs text-red-600">
              {{ errors.idNumber }}
            </p>
          </div>
          <div class="mb-4">
            <label
              for="expiryDate"
              class="block mb-1 text-sm font-medium text-gray-700"
              >วันหมดอายุบัตร <span class="text-red-500">*</span></label
            >
            <input
              type="text"
              id="expiryDate"
              ...
              class="w-full px-4 py-2 transition border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.expiryDate,
              }"
            />
          ...
          <div class="mb-4">
            <label class="block mb-1 text-sm font-medium text-gray-700"
              >รูปถ่ายใบหน้าปกติ (Selfie)
              <span class="text-red-500">*</span></label
            >
            <div
              v-if="!selfiePreview"
              @click="triggerFileUpload('selfieFile')"
              class="p-6 text-center transition border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-blue-500"
              :class="{
                'border-red-500 ring-1 ring-red-500': errors.selfieFile,
              }"
            >
              <i class="text-4xl text-gray-400 fa-regular fa-image"></i>
              <p class="mt-2 text-sm text-gray-600">กดเพื่อเลือกรูปภาพ</p>
            ...
            <input
              ...
              class="hidden"
            />
            <p v-if="errors.selfieFile" class="mt-1 text-xs text-red-600">
              {{ errors.selfieFile }}
            </p>
          </div>
          <div
            class="p-4 mb-4 text-sm text-red-700 border border-red-200 rounded-md bg-red-50"
          >
            <p class="mb-2 font-semibold">ข้อควรหลีกเลี่ยง:</p>
            <ul class="space-y-1">
              <li class="flex items-center">
                <i class="mr-2 text-red-500 fa-solid fa-circle-xmark"></i
                >ใบหน้าไม่ชัดหรือถูกบัง
              </li>
              <li class="flex items-center">
                <i class="mr-2 text-red-500 fa-solid fa-circle-xmark"></i
                >บัตรไม่ชัดหรือไม่เต็ม
              </li>
              <li class="flex items-center">
                <i class="mr-2 text-red-500 fa-solid fa-circle-xmark"></i
                >แสงน้อยเกินไป
          ...
          <div class="mb-6">
            ...
            <p v-if="errors.pdpa" class="mt-1 text-xs text-red-600">
              {{ errors.pdpa }}
            </p>
          </div>
          <div class="flex gap-4">
...
const validationFunctions = [
  async () => {
    ...
    // ถ้า format ไม่ผ่านหยุดเลย ไม่ต้อง call API
    if (Object.keys(errors).length > 0) return false;

    // ตรวจ dictionary กับ backend
    try {
      ...
      errors.password = "ไม่สามารถตรวจสอบรหัสผ่านได้ กรุณาลองใหม่";
      return false;
    }
  ...
  () => {
    ...
    if (!/^\d{13}$/.test(formData.idNumber))
      errors.idNumber = "กรุณากรอกหมายเลขบัตรประชาชน 13 หลัก";
    if (
      !formData.expiryDate.trim() ||
      !/^\d{2}\/\d{2}\/\d{4}$/.test(formData.expiryDate)
    ) {
      errors.expiryDate = "กรุณากรอกวันหมดอายุบัตรในรูปแบบ วว/ดด/ปปปป";
    } else if (!isValidDDMMYYYY(formData.expiryDate)) {
      errors.expiryDate =
        "วันที่ไม่ถูกต้อง โปรดตรวจสอบวัน/เดือน เช่น 31/04/2025 ไม่ถูกต้อง";
    }
...
const nextStep = async () => {
  const isValid = await validationFunctions[currentStep.value - 1]();
